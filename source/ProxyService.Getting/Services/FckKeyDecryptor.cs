...
public static class FckKeyDecryptor
{
    ...
    public static string DecryptPortNumbers(string source)
    {
        ...
        {
            throw new NullReferenceException("Fck keys decryptor missing initialValues or mappings");
        }
        ...
        {
            var parts = decryptedMapping.Split(['=', '^']);
            var value = 0;
            if (parts.Length == 2)
            {
